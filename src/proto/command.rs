...
impl Command {
    ...
    pub fn Join<S: Into<String>>(chans: Vec<S>, keys: Option<Vec<S>>) -> Command {
        Command::JOIN(
            chans.into_iter().map(|s| s.into()).collect(),
            keys.map(|ks| ks.into_iter().map(|s| s.into()).collect()),
        )
    }
    ...
    pub fn new(command: &str, args: Vec<&str>) -> Result<Command, ProtocolError> {
        ...
        match command.as_str() {
            ...
            "JOIN" => match args.len() {
                ...
                1 => {
                    if args[0].contains(",") {
                        Ok(Command::Join(args[0].split(",").collect(), None))
                    } else {
                        Ok(Command::Join(vec![args[0].to_owned()], None))
                    }
                }
                2 => {
...
impl<'a> From<&'a Command> for String {
    fn from(cmd: &'a Command) -> String {
        match *cmd {
            ...
            Command::JOIN(ref chans, Some(ref keys)) => {
                stringify("JOIN", &[chans.join(",").as_str(), keys.join(",").as_str()])
            }
...
mod test {
    ...
    pub fn notice() {
        ...
        let cmd = Command::new("NOTICE", src);
        assert_eq!("NOTICE * :*** Hello World ***", cmd.unwrap().to_string());
    }

    #[test]
    pub fn suffix() {
        let src = vec!["*", "*** This will be a suffix ***"];
        let cmd = Command::new("NOTICE", src);
        assert_eq!(
            "NOTICE * :*** This will be a suffix ***",
            cmd.unwrap().to_string()
        );
    }
    ...
    pub fn normal_arg() {
        let src = vec!["*", "Hello"];
        let cmd = Command::new("NOTICE", src);
        assert_eq!("NOTICE * Hello", cmd.unwrap().to_string());
    }
}
